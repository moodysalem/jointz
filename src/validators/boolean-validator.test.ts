...
describe("jointz#boolean", () => {
  it("allows true or false", () => {
    expect(jointz.boolean().validate(true)).to.deep.eq([]);
    expect(jointz.boolean().validate(false)).to.deep.eq([]);
  });

  it("rejects other types", () => {
    expect(jointz.boolean().validate("def")).to.deep.eq([
      { message: "must be a boolean", path: [], value: "def" },
    ]);
    expect(jointz.boolean().validate({})).to.deep.eq([
      { message: "must be a boolean", path: [], value: {} },
    ]);
    expect(jointz.boolean().validate(5)).to.deep.eq([
