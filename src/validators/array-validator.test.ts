...
describe("jointz#array", () => {
  it("expects arrays", () => {
    expect(jointz.array().validate({})).to.deep.eq([
      { message: "must be an array", path: [], value: {} },
    ]);
    expect(jointz.array().validate("abc")).to.deep.eq([
      { message: "must be an array", path: [], value: "abc" },
    ]);
    expect(jointz.array().validate(123)).to.deep.eq([
      { message: "must be an array", path: [], value: 123 },
    ]);
  });

  it("checks minimum length", () => {
    expect(jointz.array().minLength(1).validate([])).to.deep.eq([
      {
  ...
  it("checks maximum length", () => {
    expect(jointz.array().maxLength(1).validate(["abc", {}])).to.deep.eq([
      {
